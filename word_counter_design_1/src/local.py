...
def run_local(max_line_per_chunk, queue_size, wait_time_seconds, **queue_name):
    info_queues = queue_name["info_queue"]
    # Measure total time
    ...
    try:
        ...
        for chunk in chunks:
            send_message_to_sqs(queue_name["process_queue"], chunk)
    ...
    # Count running EC2 instances and send object location info
    num_ec2_running = len(get_ec2_info())
    for i in range(num_ec2_running):
        send_message_to_sqs(info_queues[i], run_param)
    ...
    monitor_queues(queue_name["result_queue"], 
                   num_chunks, max(min_process_chunk//num_ec2_running,1), 
                   max(num_chunks,90))
